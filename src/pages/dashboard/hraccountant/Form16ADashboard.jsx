...
import { 
    ...
    Clock, Upload, Grid, List, Check, ExternalLink, ArrowUpRight
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useTheme } from '@/context/ThemeContext';
import {
    form16ARecords as initialRecords, form16AFilingTracker as filingTracker, form16AActivityTimeline as activityTimeline
} from '@/datasets/hraccountant/form16ADashboardData';
...
const StatusBadge = ({ status }) => {
    const styles = {
        ...
        Filed: "bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 border-blue-100 dark:border-blue-500/20",
        "Not Started": "bg-gray-50 text-gray-500 dark:bg-white/5 dark:text-gray-400 border-gray-100 dark:border-white/10",
    };
    return (
...
const FilingCard = (props) => {
    const { quarter, status, date, isDarkMode } = props;
    ...
    return (
        <motion.div 
            whileHover={{ y: -4 }}
            className={`p-5 rounded-xl border transition-all duration-300 hover:shadow-lg flex flex-col gap-3 ${
                isDarkMode ? 'bg-[#0c162d]/50 border-white/10' : 'bg-white border-gray-100 shadow-sm'
            }`}
        >
            <div className="flex justify-between items-start">
                <span className="text-sm font-bold text-blue-500">{quarter}</span>
                <StatusBadge status={status} />
            </div>
            <div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">Status</p>
                <p className={`text-xs font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{status}</p>
            </div>
            <div>
            ...
            {isFiled ? (
                <div className="mt-1 flex items-center gap-1.5 text-[10px] font-bold text-emerald-500 uppercase tracking-tighter">
                    <CheckCircle size={12} /> Compliance Met
                </div>
            ) : isPending ? (
                <div className="mt-1 flex items-center gap-1.5 text-[10px] font-bold text-amber-500 uppercase tracking-tighter">
...
export default function Form16ADashboard() {
        ...
        <div className={`p-4 md:p-6 lg:p-8 space-y-6 font-body min-h-screen ${isDarkMode ? 'bg-[#0c162d]/50 text-white' : 'bg-lightBlueBg text-dark'}`}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-semibold tracking-tight uppercase">Form 16A – Quarterly TDS Certificates</h1>
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 font-medium">Quarterly Tax Compliance & Certificate Management</p>
                    </div>
                </div>
                <div className="flex gap-2 flex-wrap">
                    <button className="flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-bold transition-all active:scale-95 shadow-lg shadow-blue-500/20">
                        <Plus size={15} /> Generate Form 16A
                    </button>
                    <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 text-blue-500 border border-blue-100 dark:border-gray-700 rounded-lg text-sm font-bold transition-all hover:bg-blue-50">
                        <Upload size={15} /> Upload Filing Record
                    </button>
            ...
            <motion.div {...fadeProps} className={cardClass}>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Quarter Selection</label>
                        <div className="relative">
                            <select 
                                value={quarter} 
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Employee/Vendor Filter</label>
                        <div className="relative">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Filing Status</label>
                        <div className="relative">
                            <select 
                                value={statusFilter} 
            ...
            {/* ── 4. Quarterly Filing Tracker ───────────────────────────────── */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {filingTracker.map((item, idx) => (
                    <FilingCard key={idx} {...item} isDarkMode={isDarkMode} />
                ))}
            </div>

            {/* ── 5. Form 16A Certificate Records & 6. Preview ─────────────── */}
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-[#111827] border-b border-gray-100 dark:border-gray-800 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                    <th className="px-6 py-4">Employee / Vendor</th>
                                    <th className="px-4 py-4">Dept / ID</th>
                                    ...
                                    >
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] font-medium text-gray-400 uppercase tracking-tighter">{row.email}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-xs font-bold text-blue-500">{row.q}</span>
                                            <p className="text-[10px] text-gray-400">{row.fy}</p>
                                        </td>
                                        <td className="px-4 py-4">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="aspect-[3/4] bg-gray-50 dark:bg-[#111827] rounded-xl border border-dashed border-gray-200 dark:border-gray-800 flex flex-col items-center justify-center p-8 text-center group transition-all">
                                ...
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed">
                                    {selectedRecord?.status === 'Filed' || selectedRecord?.status === 'Generated' 
                                        ? `Quarterly TDS Certificate for ${selectedRecord?.name}` 
                                        : "Certificate Not Available for Preview"}
                                </p>
                                {selectedRecord?.status === 'Filed' && (
                                    <div className="mt-4 flex items-center gap-2 text-[10px] font-bold text-emerald-500 bg-emerald-500/5 px-3 py-1.5 rounded-full border border-emerald-500/10 transition-all group-hover:bg-emerald-500 group-hover:text-white">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between py-2 border-b border-dashed border-gray-100 dark:border-gray-800">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase">Quarter</span>
                                    <span className="text-xs font-bold text-blue-500">{selectedRecord?.q} ({selectedRecord?.fy})</span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-dashed border-gray-100 dark:border-gray-800">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase">Filing Status</span>
                            ...
                            <div className="space-y-2 pt-4">
                                ...
                                <button className="w-full py-2.5 text-blue-500 text-xs font-bold hover:underline transition-all">
            ...
            {/* ── 7. Certificate Distribution Panel ────────────────────────── */}
            <motion.div {...fadeProps} className={cardClass}>
                <div className="flex items-center gap-2 mb-6">
                    <Mail size={18} className="text-blue-500" />
                    <h3 className="text-sm font-bold uppercase tracking-tight">Certificate Distribution Tracker</h3>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                            {initialRecords.map((row, idx) => (
                                <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-all text-xs">
                                    ...
                                    <td className="px-4 py-4 text-center font-bold">{row.q}</td>
            ...
            {/* ── 8. Compliance Timeline & Quick Actions ───────────────────── */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-12">
                {/* Activity Feed */}
                <motion.div {...fadeProps} className={cardClass}>
                    <div className="flex items-center gap-2 mb-6">
                        <History size={18} className="text-blue-500" />
                        <h3 className="text-sm font-bold uppercase tracking-tight">Quarterly Compliance Activity</h3>
                    </div>
                        ...
                        {activityTimeline.map((item, i) => (
                            <div key={i} className="relative flex items-start gap-4 group">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[11px] font-bold leading-tight group-hover:text-blue-500 transition-colors uppercase tracking-tight">{item.text}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <Clock size={10} className="text-gray-400" />
                                        <span className="text-[10px] font-medium text-gray-400 font-mono tracking-tighter">{item.time}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <motion.div {...fadeProps} className={`lg:col-span-2 ${cardClass}`}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            { label: 'Generate Quarterly Certificates', icon: FileCheck, color: 'bg-blue-500' },
                            { label: 'Submit Filing Record', icon: ShieldCheck, color: 'bg-emerald-500' },
                            { label: 'Bulk Email Certificates', icon: Mail, color: 'bg-amber-500' },
                            { label: 'Export Certificate Data', icon: FileDown, color: 'bg-purple-500' },
                        ].map((btn, i) => (
                            <button 
                                key={i} 
                    ...
                    <div className="mt-8 p-4 bg-emerald-50/50 dark:bg-emerald-500/5 rounded-xl border border-dashed border-emerald-200 dark:border-emerald-500/20 flex gap-4 items-center">
                        <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center shrink-0">
                            <ShieldCheck size={20} className="text-emerald-500" />
                        </div>
                        <p className="text-[11px] font-medium leading-relaxed italic text-emerald-600/80 dark:text-emerald-400/60">
                            <span className="font-bold text-emerald-600 dark:text-emerald-400 uppercase mr-1">Compliance Check:</span>
                            Form 16A certificates must be issued within 15 days from the due date for furnishing the statement of tax deducted at source.
                        </p>
                    </div>
                </motion.div>
            </div>
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 p-1 bg-white/40 dark:bg-[#1f2937]/40 backdrop-blur-xl rounded-full shadow-2xl border border-white/20 dark:border-gray-800/20 z-50">
                <div className="flex items-center gap-2 px-5 py-2 bg-emerald-500 text-white rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-500/20">
                    <CheckCircle size={14} className="animate-pulse" />
                    Quarterly Compliance Active
                </div>
                <div className="flex items-center gap-6 px-4 pr-8 py-2">
                    <div className="flex flex-col items-center">
                        <span className="text-[8px] font-bold text-blue-500 uppercase leading-none mb-1">Filed</span>
                        <span className="text-sm font-black tracking-tighter">100% (Q2)</span>
                    </div>
                    <div className="w-px h-6 bg-gray-300 dark:bg-gray-700" />
                    <div className="flex flex-col items-center">
                        <span className="text-[8px] font-bold text-amber-500 uppercase leading-none mb-1">Q3 Status</span>
                        <span className="text-sm font-black tracking-tighter italic">Pending</span>
                    </div>
                </div>
            </div>
        </div>
